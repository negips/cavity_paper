...
\begin{subequations}
\begin{align}
	\mathcal{Y}(\vecparv) = \sum \vecybar_{i}\parv_{i} + \sum \sum\vecybar_{i,j}\parv_{i}\parv_{j} \ldots, \\ 
	\mathcal{G}(\vecparv) = \sum \mathbf{g}_{i}\parv_{i} + \sum \sum\mathbf{g}_{i,j}\parv_{i}\parv_{j} \ldots,
\end{align}
\end{subequations}
...
\begin{aligned}
	...
	%
	g^{3}_{3} =& (\sigma' + 10.31\iota) + (1.801 + 1.096\iota)\epsilon \nonumber \\
	& + (0.631 + 0.395\iota)\epsilon^{2} - (0.005 - 0.001\iota)\epsilon\sigma', \nonumber 
\end{aligned}
\end{equation}
\begin{equation}
	\begin{aligned}
...
The author is grateful for the computational resources and support provided by the Scientific Computing and Data Analysis section of Research Support Division at OIST. The author would also like to thank Dr. Ardeshir Hanifi for his comments on the manuscript.
