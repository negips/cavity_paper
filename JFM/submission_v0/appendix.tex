...
Table~\ref{tab:coeffs_x1} reports the evaluated coefficients for the different polynomial terms in the center-manifold amplitude equations for the Hopf bifurcations in the cylinder wake and for the flow in an open cavity. The reported coefficient are for $\dt{x}$ and one obtains the complex conjugated terms for $\dt{x}^{*}$ up to an accurary of $10^{-8}$, where numerical accuracy and solver tolerances cause a violation of the complex conjugation property.
