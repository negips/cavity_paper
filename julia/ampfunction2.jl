...
# 1st order
# |z3|^2
ntt    = CenterManifold.NInteractionTerms(0,NV0)
v34_O1 = zeros(Float64,ntt)
for i in 1:ntt
  ord       = 0
  indfinal  = [ind1; ind2] .- 1
  ntmp      = CenterManifold.GetIndexNumber(indfinal,NV)
  v34_O1[i] = real.(H34[2][ind1,ntmp])
end  

# 2nd order
# ϵ*|z3|^2
# σ'*|z3|^2
ntt    = CenterManifold.NInteractionTerms(1,NV0)
v34_O2 = zeros(Float64,ntt)
for i in 1:ntt
  ord       = 1
  ind0      = CenterManifold.GetPolynomialIndices(i,ord,NV0) .+ 1
  ind00     = NV0Ind[ind0]
  indfinal  = [ind00; ind1; ind2] .- 1
  ntmp      = CenterManifold.GetIndexNumber(indfinal,NV)
  v34_O2[i] = real.(H34[3][ind1,ntmp])
end  
...
# ϵ*σ'*|z3|^2
# σ'^2*|z3|^2
ntt    = CenterManifold.NInteractionTerms(2,NV0)
v34_O3 = zeros(Float64,ntt)
for i in 1:ntt
  ord       = 2
  ind0      = CenterManifold.GetPolynomialIndices(i,ord,NV0) .+ 1
  ind00     = NV0Ind[ind0]
...
# ϵ*σ^2*|z3|^2
# σ^3*|z3|^2
ntt    = CenterManifold.NInteractionTerms(3,NV0)
v34_O4 = zeros(Float64,ntt)
for i in 1:ntt
  ord       = 3
  ind0      = CenterManifold.GetPolynomialIndices(i,ord,NV0) .+ 1
  ind00     = NV0Ind[ind0]
...
# |z3|^2*|z3|^2
v34_LO3    = zeros(Float64,1)
itmp       = [ind1; ind2; ind1; ind2] .- 1
n3_1       = CenterManifold.GetIndexNumber(itmp,NV)
v34_LO3[1] = real.(H34[4][ind1,n3_1])

# Cross-Term
# |z5|^2*|z3|^2
v34_LCO3    = zeros(Float64,1)
itmp        = [ind3; ind4; ind1; ind2] .- 1
n3_2        = CenterManifold.GetIndexNumber(itmp,NV)
v34_LCO3[1] = real.(H34[4][ind1,n3_2])

# 4th Order
# ϵ*|z3|^2*|z3|^2
# σ*|z3|^2*|z3|^2
ntt     = CenterManifold.NInteractionTerms(1,NV0)
v34_LO4 = zeros(Float64,ntt)
for i in 1:ntt
  ord        = 1
  ind0       = CenterManifold.GetPolynomialIndices(i,ord,NV0) .+ 1
  ind00      = NV0Ind[ind0]
  indfinal   = [ind00; ind1; ind2; ind1; ind2] .- 1
  ntmp       = CenterManifold.GetIndexNumber(indfinal,NV)
  v34_LO4[i] = real.(H34[5][ind1,ntmp])
end  
...
# σ*|z5|^2*|z3|^2
ntt      = CenterManifold.NInteractionTerms(1,NV0)
v34_LCO4 = zeros(Float64,ntt)
for i in 1:ntt
  ord         = 1
  ind0        = CenterManifold.GetPolynomialIndices(i,ord,NV0) .+ 1
  ind00       = NV0Ind[ind0]
  indfinal    = [ind00; ind3; ind4; ind1; ind2] .- 1
  ntmp        = CenterManifold.GetIndexNumber(indfinal,NV)
  v34_LCO4[i] = real.(H34[5][ind1,ntmp])
end  
